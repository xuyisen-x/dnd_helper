...
fn process_primary(pair: pest::iterators::Pair<Rule>) -> Expr {
    match pair.as_rule() {
        Rule::dice_expr => {
            ...
            match first.as_rule() {
                Rule::dice_op => { // 以dice_op开头，省略了数量，则默认为1
                    let atom_pair = inner_pairs.next().unwrap();
                    let sides = parse_atom(atom_pair);
                    Expr::Dice(Box::new(Expr::Number(1.0)), Box::new(sides))
                }
                Rule::atom => { // 以atom开头，说明有数量，可能是单纯的数值或者ndn的表达式
                    let count_or_number = parse_atom(first);
                    match inner_pairs.next() {
                        Some(_) => { // 后面跟着dice_op，说明是ndn表达式
                            let sides_pair = inner_pairs.next().unwrap();
                            let sides = parse_atom(sides_pair);
                            Expr::Dice(Box::new(count_or_number), Box::new(sides))
                        }
                        None => { // 只有一个atom，直接返回
                            count_or_number
                        }
...
fn process_infix(lhs: Expr, op: pest::iterators::Pair<Rule>, rhs: Expr) -> Expr {
    ...
    Expr::Binary(Box::new(lhs), bin_op, Box::new(rhs))
}

fn process_prefix(op: pest::iterators::Pair<Rule>, rhs: Expr) -> Expr {
    match op.as_rule() {
        Rule::neg => Expr::Binary(Box::new(Expr::Number(0.0)), BinOp::Sub, Box::new(rhs)),
        Rule::pos => rhs, // 正号不做处理
...
fn process_postfix(lhs: Expr, op: pest::iterators::Pair<Rule>) -> Expr {
    ...
    match op.as_rule() {
        ...
        Rule::drop_low => {
            ...
            Expr::Modifier { lhs: Box::new(lhs), op: op_enum, param: param }
        }
        Rule::reroll_once |
        Rule::reroll |
        Rule::explode_compound |
        Rule::explode => {
            let op_enum  = match op.as_rule() {
            ...
            let param = if let Some(mod_param) = inner_pairs.next() {
                ...
                match first.as_rule() {
                    Rule::atom => { // 是值，默认op为等于
                        let value = parse_atom(first);
                        Some(ModifierParam::Compare(
                            CompareExpr {
                                op: CompareOp::Equal,
                                val: Box::new(value)
                            }
    ...
    match inner_pairs.as_rule() {
        ...
        Rule::function => {
            ...
            let args = match inner.next() {
                Some(args_pair) => args_pair.into_inner().map(|p| parse_expr_pratt(p)).collect(),
                None => vec![],
            };
            Expr::Call(name, args)
        },
        ...
        // 容错处理
        _ => unreachable!("Unknown atom: {:?} - {}", inner_pairs.as_rule(), inner_pairs.as_str())
    }
}